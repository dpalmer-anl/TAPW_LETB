...
def set_relative_dis_ndarray(atom_pstn_list: np.ndarray, enlarge_atom_pstn_list: np.ndarray,
    ...
    """set up relative distance between one atom and another
    ...
    di = np.zeros(num_atoms)
    dj = np.zeros(num_atoms)

    area1_di = np.ones(num_atoms)
    area2_di = np.zeros(num_atoms)
    area3_di = -1*np.ones(num_atoms)
    area4_di = np.zeros(num_atoms)
    area5_di = np.ones(num_atoms)
    area6_di = np.ones(num_atoms)
    area7_di = -1*np.ones(num_atoms)
    area8_di = -1*np.ones(num_atoms)

    area1_dj = np.zeros(num_atoms)
    area2_dj = np.ones(num_atoms)
    area3_dj = np.zeros(num_atoms)
    area4_dj = -1*np.ones(num_atoms)
    area5_dj = np.ones(num_atoms)
    area6_dj = -1*np.ones(num_atoms)
    area7_dj = np.ones(num_atoms)
    area8_dj = -1*np.ones(num_atoms)
    enlarge_di = np.concatenate((di, area1_di, area2_di, area3_di, area4_di, area5_di, area6_di, area7_di, area8_di))
    enlarge_dj = np.concatenate((dj, area1_dj, area2_dj, area3_dj, area4_dj, area5_dj, area6_dj, area7_dj, area8_dj))
    ...
    row_di = enlarge_di[np.hstack(all_nns)]
    col_dj = enlarge_dj[np.hstack(all_nns)]
    ...
    npair_dict['r'] = row
    npair_dict['c'] = col
    npair_dict['di'] = row_di
    npair_dict['dj'] = col_dj

    # first two dimentions (dri -drj)
    dr = (atom_pstn_2darray-atom_neighbour_2darray)[:, :2]
    # the third dimention  (dri -drj)
    dd = (atom_pstn_2darray-atom_neighbour_2darray)[:, -1]
    # neighour distance dict
    ndist_dict = {}
    ndist_dict['dr'] = dr
    ndist_dict['dd'] = dd
    ndist_dict['disp'] = (atom_pstn_2darray-atom_neighbour_2darray)[:, :]
    ndist_dict["natoms"] = num_atoms
